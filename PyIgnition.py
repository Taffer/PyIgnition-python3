...
from constants import DRAWTYPE_BUBBLE, DRAWTYPE_CIRCLE, DRAWTYPE_IMAGE, \
    DRAWTYPE_LINE, DRAWTYPE_POINT, DRAWTYPE_SCALELINE, \
    INTERPOLATIONTYPE_COSINE, INTERPOLATIONTYPE_LINEAR, PYIGNITION_VERSION, \
    RADIUS_PERMITTIVITY
...
class ParticleEffect:
    ...
    def Update(self):
        ...
        for this_gravity in self.gravities:
            this_gravity.Update()
        ...
        for particle in self.particles:
            ...
            if particle.drawtype == DRAWTYPE_CIRCLE or \
               particle.drawtype == DRAWTYPE_BUBBLE:
                # ...But only set if the particle is actually circular
                radius = particle.radius * (1.0 - RADIUS_PERMITTIVITY)

            # First calculate the forces acting on the particle
            totalforce = [0.0, 0.0]

            for this_gravity in self.gravities:
                force = this_gravity.GetForceOnParticle(particle)
            ...
            particle.velocity = [particle.velocity[0] + totalforce[0],
                                 particle.velocity[1] + totalforce[1]]

            particle.Update()

            # Resolve collisions
            for obstacle in self.obstacles:
                if (not obstacle.OutOfRange(particle.pos)) and \
                   (obstacle.InsideObject(particle.pos, radius)):
    ...
    def CreateSource(self, pos=(0, 0), initspeed=0.0, initdirection=0.0,
                     initspeedrandrange=0.0, initdirectionrandrange=0.0,
                     particlesperframe=0, particlelife=-1, genspacing=0,
                     drawtype=0, colour=(0, 0, 0), radius=0.0, length=0.0,
                     imagepath=None):
        newsource = particles.ParticleSource(self, pos, initspeed,
                                             initdirection, initspeedrandrange,
    ...
    def CreatePointGravity(self, strength=0.0, strengthrandrange=0.0,
                           pos=(0, 0)):
        newgrav = gravity.PointGravity(strength, strengthrandrange, pos)
        self.gravities.append(newgrav)
        return newgrav

    def CreateDirectedGravity(self, strength=0.0, strengthrandrange=0.0,
                              direction=[0, 1]):
        newgrav = gravity.DirectedGravity(strength, strengthrandrange,
                                          direction)
        self.gravities.append(newgrav)
        return newgrav

    def CreateVortexGravity(self, strength=0.0, strengthrandrange=0.0,
                            pos=(0, 0)):
        newgrav = gravity.VortexGravity(strength, strengthrandrange, pos)
        self.gravities.append(newgrav)
        return newgrav

    def CreateCircle(self, pos=(0, 0), colour=(0, 0, 0), bounce=1.0,
                     radius=0.0):
        newcircle = obstacles.Circle(self, pos, colour, bounce, radius)
        self.obstacles.append(newcircle)
        return newcircle

    def CreateRectangle(self, pos=(0, 0), colour=(0, 0, 0), bounce=1.0,
                        width=0.0, height=0.0):
        newrect = obstacles.Rectangle(self, pos, colour, bounce, width, height)
        self.obstacles.append(newrect)
        return newrect

    def CreateBoundaryLine(self, pos=(0, 0), colour=(0, 0, 0), bounce=1.0,
                           normal=[0, 1]):
    ...
    # I'm not reformatting this for 80 characters. - Taffer
    def SaveToFile(self, outfilename):
        ...
        for this_gravity in self.gravities:
            ...
            for keyframe in this_gravity.keyframes:
                if keyframe.frame == 0:  # Don't bother writing out the first keyframe
                    continue
            ...
            if child.tag == "source":  # Source object
                ...
                for keyframe in keyframes:
                    ...
                    for variable in keyframe.children:
                        if variable.tag == "pos_x" and \
                           variable.inside != "None":
                            variables['pos_x'] = int(variable.inside)
                        elif variable.tag == "pos_y" and \
                            variable.inside != "None":  # noqa: E125
                            variables['pos_y'] = int(variable.inside)
                        elif variable.tag == "initspeed" and \
                            variable.inside != "None":  # noqa: E125
                            variables['initspeed'] = float(variable.inside)
                        elif variable.tag == "initdirection" and \
                            variable.inside != "None":  # noqa: E125
                            variables['initdirection'] = float(variable.inside)
                        elif variable.tag == "initspeedrandrange" and \
                            variable.inside != "None":  # noqa: E125
                            variables['initspeedrandrange'] = float(
                                variable.inside)
                        elif variable.tag == "initdirectionrandrange" and \
                            variable.inside != "None":  # noqa: E125
                            variables['initdirectionrandrange'] = float(
                                variable.inside)
                        elif variable.tag == "particlesperframe" and \
                            variable.inside != "None":  # noqa: E125
                            variables['particlesperframe'] = int(
                                variable.inside)
                        elif variable.tag == "genspacing" and \
                            variable.inside != "None":  # noqa: E125
                            variables['genspacing'] = int(variable.inside)
                        elif variable.tag == "interpolationtype" and \
                            variable.inside != "None":  # noqa: E125
                            variables['interpolationtype'] = \
                                self.GetStringAsInterpolationtype(
                                    variable.inside)
                ...
                for keyframe in keyframes:
                    ...
                    for variable in keyframe.children:
                        if variable.tag == "strength" and \
                            variable.inside != "None":  # noqa: E125
                            variables['strength'] = float(variable.inside)
                        elif variable.tag == "strengthrandrange" and \
                            variable.inside != "None":  # noqa: E125
                            variables['strengthrandrange'] = \
                                float(variable.inside)
                        elif variable.tag == "direction_x" and \
                            variable.inside != "None":  # noqa: E125
                            variables['direction_x'] = float(variable.inside)
                        elif variable.tag == "direction_y" and \
                            variable.inside != "None":  # noqa: E125
                ...
                for keyframe in keyframes:
                    ...
                    for variable in keyframe.children:
                        if variable.tag == "pos_x" and \
                            ...
                            variable.inside != "None":  # noqa: E125
                            variables['pos_y'] = int(variable.inside)
                        elif variable.tag == "colour_r" and \
                            variable.inside != "None":  # noqa: E125
                            variables['colour_r'] = int(variable.inside)
                        elif variable.tag == "colour_g" and \
                            variable.inside != "None":  # noqa: E125
                ...
                for keyframe in keyframes:
                    ...
                    for variable in keyframe.children:
                        if variable.tag == "pos_x" and \
                            ...
                            variable.inside != "None":  # noqa: E125
                            variables['bounce'] = float(variable.inside)
                        elif variable.tag == "width" and \
                            variable.inside != "None":  # noqa: E125
                            variables['width'] = float(variable.inside)
                        elif variable.tag == "height" and \
                            variable.inside != "None":  # noqa: E125
                ...
                for keyframe in keyframes:
                    ...
                    for variable in keyframe.children:
                        if variable.tag == "pos_x" and \
                            ...
                            variable.inside != "None":  # noqa: E125
                            variables['bounce'] = float(variable.inside)
                        elif variable.tag == "normal_x" and \
                            variable.inside != "None":  # noqa: E125
                            variables['normal_x'] = float(variable.inside)
                        elif variable.tag == "normal_y" and \
                            variable.inside != "None":  # noqa: E125
...
if __name__ == '__main__':
    ...
    testsource = test.CreateSource((10, 10), initspeed=5.0,
