...
class Obstacle:
    def __init__(self, parenteffect, pos, colour, bounce):
        ...
        self.colour = colour
        self.bounce = bounce
        # The maximum (square-based, not circle-based) distance away for which
        # forces will still be calculated
    ...
    def OutOfRange(self, pos):
        return (abs(pos[0] - self.pos[0]) > self.maxdist) or \
               (abs(pos[1] - self.pos[1]) > self.maxdist)
    ...
    # Gets the normal relevant to a particle at the supplied potision (for
    # example, that of the appropriate side of a squre)
    def GetNormal(self, pos):
        pass

    # Gets the force as a factor of maximum available force (0.0 - 1.0),
    # determined by an inverse cube distance law
    def GetForceFactor(self, pos, pradius):
        pass

    # Gets the final (vector) force
    def GetForce(self, pos, velocity, pradius=0.0):
        ...
        # An integer between 0.0 and 1.0 used to ensure force is maximised for
        # head-on collisions and minimised for scrapes
        scalingfactor = -dotproduct2d(normal, velocity)

        # The force should never be attractive, so take any negative value of
        # the scaling factor to be zero
        if scalingfactor <= 0.0:
            # A scaling factor of zero always results in zero force
            return [0.0, 0.0]
        ...
        # Force = bounce factor * velocity * distance force factor (0.0 - 1.0) *
        # angle force factor (0.0 - 1.0), along the direction of the normal
        # pointing away from the obstacle
        return [normal[0] * forcefactor * velmag * scalingfactor * self.bounce,
                normal[1] * forcefactor * velmag * scalingfactor * self.bounce]
...
class Circle(Obstacle):
    ...
    def Draw(self, display):
        ...
        pygame.draw.circle(display, self.colour, (offset[0] + int(self.pos[0]),
                                                  offset[1] + int(self.pos[1])),
                           int(self.radius))
    ...
    def GetResolved(self, pos, pradius=0.0):
        # If the position is at this obstacle's origin, shift it up a pixel to
        ...
        # Split the pradius into appropriate components by multiplying the
        # normal vector by pradius
        pradiusvec = [(nor[0] * pradius), (nor[1] * pradius)]

        correctedvec = [nor[0] * (self.radius), nor[1] * (self.radius)]

        return [self.pos[0] + correctedvec[0] + pradiusvec[0],
                self.pos[1] + correctedvec[1] + pradiusvec[1]]
    ...
    def GetForceFactor(self, pos, pradius=0.0):
        ...
        distcubed = (abs(pow(float(newpos[0] - self.pos[0]), 3.0)) +
                     abs(pow(float(newpos[1] - self.pos[1]), 3.0))) - \
    ...
    def CreateKeyframe(self, frame, pos=(None, None), colour=(None, None, None),
                       bounce=None, radius=None,
...
class Rectangle(Obstacle):
    def __init__(self, parenteffect, pos, colour, bounce, width, height):
        ...
        self.CreateKeyframe(0, self.pos, self.colour, self.bounce, self.width,
                            self.height)

    def Draw(self, display):
        offset = self.parenteffect.pos
        pygame.draw.rect(display, self.colour,
                         pygame.Rect(offset[0] + (self.pos[0] - self.halfwidth),
                                     offset[1] + (self.pos[1] - self.halfheight),
    ...
    def InsideObject(self, pos, pradius=0.0):
        return (((pos[0] + pradius) > (self.pos[0] - self.halfwidth)) and
                ((pos[0] - pradius) < (self.pos[0] + self.halfwidth)) and
                ((pos[1] + pradius) > (self.pos[1] - self.halfheight)) and
                ((pos[1] - pradius) < self.pos[1] + self.halfheight))
    ...
    def GetResolved(self, pos, pradius=0.0):
        ...
        # Where 'triangles' within the rectangle are referred to, imagine a
        # rectangle with diagonals drawn between its vertices. The four
        # triangles formed by this process are the ones referred to
        #
        # If it's directly above the centre of the rectangle
        if pos[0] == self.pos[0]:
            if pos[1] > self.pos[1]:
                return [pos[0], self.pos[1] + self.halfheight + pradius]
            else:
                return [pos[0], self.pos[1] - (self.halfheight + pradius)]
        # If it's directly to one side of the centre of the rectangle
        elif pos[1] == self.pos[1]:
            if pos[0] > self.pos[0]:
                return [self.pos[0] + self.halfwidth + pradius, pos[1]]
            else:
                return [self.pos[0] - (self.halfwidth + pradius), pos[1]]
        # If it's in the upper or lower triangle of the rectangle
        elif abs(float(pos[1] - self.pos[1]) /
                 float(pos[0] - self.pos[0])) > (float(self.height) /
                                                 float(self.width)):
            # Halfheight is multiplied by a normalised version of
            # (pos[1] - self.pos[1]) - thus if (pos[1] - self.pos[1]) is
            # negative, it should be subtracted as the point is in the upper
            # triangle
            return [pos[0], self.pos[1] + ((self.halfheight + pradius) *
                                           ((pos[1] - self.pos[1]) /
                                               abs(pos[1] - self.pos[1])))]
        else:  # If it's in the left or right triangle of the rectangle
            return [self.pos[0] + ((self.halfwidth + pradius) *
                                   ((pos[0] - self.pos[0]) / abs(pos[0] -
                                    self.pos[0]))), pos[1]]
    ...
    def GetForceFactor(self, pos, pradius=0.0):
        ...
        if nor[0] == 0:
            if (pos[0] > (self.pos[0] - self.halfwidth)) and \
                (pos[0] < (self.pos[0] + self.halfwidth)):  # noqa: E125
                r = (abs(pos[1] - self.pos[1]) - self.halfheight) - pradius
            else:
                return 0.0
        elif nor[1] == 0:
            if (pos[1] > (self.pos[1] - self.halfheight)) and \
                (pos[1] < (self.pos[1] + self.halfheight)):  # noqa: E125
    ...
    def CreateKeyframe(self, frame, pos=(None, None), colour=(None, None, None),
                       bounce=None, width=None, height=None,
                       interpolationtype=INTERPOLATIONTYPE_LINEAR):
        values = {
            'pos_x': pos[0],
            'pos_y': pos[1],
...
class BoundaryLine(Obstacle):
    ...
    def Draw(self, display):
        ...
        if not self.hascontacts:
            ...
            if self.normal[0] == 0.0:
                ...
                pdotn = (self.pos[0] * self.normal[0]) + \
                        (self.pos[1] * self.normal[1])
        ...
        for contact in self.edgecontacts:
            tempedgecontacts.append([offset[0] + contact[0],
                                     offset[1] + contact[1]])

        if len(tempedgecontacts) >= 2:
            pygame.draw.aalines(display, self.colour, True, tempedgecontacts)
        else:
            # The line must be completely outwith the boundaries of the display
            pass
    ...
    def InsideObject(self, pos, pradius=0.0):
        # If the particle has no radius, just test whether its centre position
        # has crossed the line
        if pradius == 0.0:
            return (((float(pos[0] - self.pos[0]) * self.normal[0]) +
                     (float(pos[1] - self.pos[1]) * self.normal[1])) <= 0.0)
        ...
        return ((((float(leftside[0] - self.pos[0]) * self.normal[0]) +
                  (float(leftside[1] - self.pos[1]) * self.normal[1])) <= 0.0)
                or (((float(rightside[0] - self.pos[0]) * self.normal[0]) +
                     (float(rightside[1] - self.pos[1]) * self.normal[1])) <= 0.0))
