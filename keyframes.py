...
def CreateKeyframe(parentframes, frame, variables):
    ...
    try:
        oldkey = next((keyframe for keyframe in parentframes if
                       keyframe.frame == frame))
    except StopIteration:
        oldkey = None

    if oldkey is not None:
        # For every variable defined by the old keyframe
        for var in list(oldkey.variables.keys()):
            # If a variable is undefined, copy its value to the new keyframe
            if (var not in newframe.variables or
                newframe.variables[var] is None) and (oldkey.variables[var]
                                                      is not None):
                newframe.variables[var] = oldkey.variables[var]

    # Remove the duplicate keyframe, if it existed
    for duplicate in (keyframe for keyframe in parentframes if
                      keyframe.frame == frame):
    ...
    # Return a reference to the new keyframe, in case it's needed
    return newframe
